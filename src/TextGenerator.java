        ...
        for (int i = 0; i < fileList.length; i++){
            if (fileList[i] != null){
                ...
                if (frameSize != null){
                    ...
                    try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputText, true))) {
                        writer.write("\t[\"Interface\\\\AddOns\\\\TwitchEmotes\\\\Emotes\\\\Buhs\\\\" + fileList[i].substring(0, fileList[i].length() - 5) +
    ...
    static void FirstPart(String[] fileList, String outputText) {
        for (String s : fileList) {
            if (s != null) {
                ...
                try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputText, true))) {
                    writer.write("\t[\"" + s.substring(0, s.length() - 5) + "\"] = \"Interface\\\\AddOns\\\\TwitchEmotes\\\\Emotes\\\\Buhs\\\\" + s.substring(0, s.length() - 5) + ".tga:" + standardHeight + ":" + width +"\",");
                    writer.newLine();
                } catch (IOException e) {
                    System.err.println(e.getMessage());
                }
